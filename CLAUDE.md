...
- **`scheduler.py`** picks a random unused topic from `topics/*.txt` (each file name is a category), calls `writer.generate_article()` directly, and is intended to drive unattended daily generation via cron (see `README.md`). If every topic in every category has already been used, it falls back to `topic_researcher.pesquisar_novos_temas()` to generate fresh topics via the LLM before picking one.
