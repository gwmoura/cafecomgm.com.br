...
python3 scheduler.py    # pick a random topic from topics/*.txt and generate a post (used by cron)
...
- **`scheduler.py`** picks a random line from a random file in `topics/*.txt` (each file name is a category) and is intended to drive unattended daily generation via cron (see `README.md`); it currently invokes a `main.py` that does not exist in this directory, so check before relying on it as-is.
