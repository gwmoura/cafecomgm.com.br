...
- Live deploys happen via Cloudflare Pages/Workers, which auto-builds and publishes on push to `master` — no manual deploy step needed. The `[deployment]` GCS target in config.toml is legacy/unused; don't treat it as the real deploy mechanism.
...
hugo                    # build the static site into public/ (Cloudflare does this on push, but useful to preview locally)
```
